...
# Pre-compiled suspicious regex patterns
...
# Persistent magic instance (reduces overhead)
...
def analyze_pe_file(file_path):
    """Analyze PE file by only loading headers & first 4KB."""
    try:
        with open(file_path, "rb") as f:
            file_data = f.read(4096)  # Read only first 4KB

        pe = pefile.PE(file_path, fast_load=True)  # Load headers only
...
def analyze_script_file(file_path):
    """Analyze script files for suspicious patterns efficiently."""
    try:
        with open(file_path, "rb") as f:
...
def analyze_generic_file(file_path):
    """Analyze non-PE and non-script files for high entropy."""
    try:
        with open(file_path, "rb") as f:
            file_data = f.read(4096)  # Read only first 4KB
