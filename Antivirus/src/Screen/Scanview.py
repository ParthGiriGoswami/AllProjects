...
def ScanView(page: ft.Page,rule,quickfiles,deepfiles):
    ...
    return ft.Container(
        ...
        content=ft.Column(
            [
                create_custom_button(page,"Quick Scan","Quickly scans high-risk areas for threats",100,icon=ft.Icons.SAVED_SEARCH,on_click=lambda _:Scan(page,quickfiles,rule,False)),
                create_custom_button(page,"Deep Scan","A full threat inspection for your entire device",100,icon=ft.Icons.SCREEN_SEARCH_DESKTOP_SHARP,on_click=lambda _:Scan(page,deepfiles,rule,True)),
